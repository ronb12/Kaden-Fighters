#!/usr/bin/env python3
"""
Build assets/astra_raijin.png (1376×768 ASTRA) from hand-authored Raijin reference PNGs
in assets/raijin_concept/. Re-run after updating those source files.
"""
...
CELL_H = 384
COLS = 5
RW = 275  # cell width col 0..3
LAST_W = 276  # col 4

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
KADEN = os.path.join(ROOT, "assets", "astra_fighter_sheet.png")
OUT = os.path.join(ROOT, "assets", "astra_raijin.png")
SRC4 = os.path.join(ROOT, "assets", "raijin_concept", "raijin_4poses.png")
TURN = os.path.join(ROOT, "assets", "raijin_concept", "raijin_turnaround.png")
FULL = os.path.join(ROOT, "assets", "raijin_concept", "raijin_full_sheet.png")
...
def blit_in_cell(
    ...
    if glow:
        ...
        d = ImageDraw.Draw(g)
        d.ellipse((2, nh - 8, nw + 22, nh + 24), fill=(100, 180, 255, 100))
        g.paste(f, (0, 0), f)
        f = g
    px = x0 + (sw - f.size[0]) // 2
    py = y0 + sh - f.size[1] - 6
...
def extract_action_strip(fim: Image.Image) -> list[Image.Image]:
    """Tall sheet: right ~48% of width, upper-mid Y band = labelled action row."""
    w, h = fim.size
    x0, x1 = int(w * 0.50), w
...
def main() -> int:
    for p in (SRC4, TURN, FULL):
        if not os.path.exists(p):
            print("Missing", p, file=sys.stderr)
            return 1
    f4 = Image.open(SRC4).convert("RGB")
    ft = Image.open(TURN).convert("RGB")
    ff = Image.open(FULL).convert("RGB")

    # 4 main poses; avoid bottom caption band
    poses = split_row(f4, 4, h_frac=0.68)
    turns = split_row(ft, 4, h_frac=0.64)
    act = extract_action_strip(ff)
    if len(act) < 5:
        act = [poses[1], poses[0], poses[2], poses[3], poses[3]]
    ...
    for r in range(2):
        ...
        for c in range(COLS):
            draw_checker(
                out, cell_x(c), y0, cell_w(c), CELL_H, ca, cb
            )

    # Row0: idle, idle2, walk, walk, jump
    # idle, idle2: first two 4-poses, or blend turn for idle2
    blit_in_cell(out, 0, 0, poses[0])
    blit_in_cell(
        out, 1, 0, turns[0] if len(turns) > 0 else poses[0]
    )  # 3/4 = idle2 feel
    blit_in_cell(
        out, 2, 0, turns[1] if len(turns) > 1 else poses[1]
    )
    blit_in_cell(
        out, 3, 0, turns[2] if len(turns) > 2 else poses[2]
    )
    # jump: knee-up or profile
    blit_in_cell(
        out, 4, 0, poses[1] if len(poses) > 1 else poses[0]
    )

    # Row1: jab, cross, kick, special, super — prefer extracted action row
    if len(act) >= 5:
        for i in range(5):
            blit_in_cell(
                out, i, 1, act[i], hflip=(i == 1), glow=(i == 4)
            )
    else:
        blit_in_cell(out, 0, 1, poses[0])
        blit_in_cell(out, 1, 1, poses[0], hflip=True)
        blit_in_cell(out, 2, 1, poses[2])
        blit_in_cell(out, 3, 1, poses[3])
        blit_in_cell(out, 4, 1, poses[3], glow=True)
